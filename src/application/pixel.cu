...
int main() {
  ...
  for (;;) {
    ...
    for (int j = 0; j < Frames; j++) {
      ...
      cv::Mat build(cv::Size(width, height),
                    CV_8UC1,
                    grayData); // NEED TO CHANGE TO GPU DATA IF USING CUDAMALLOC AND NOT CUDAMALLOCMANAGED
...
/**
  ...
  //cv::Mat_<cv::Vec3b> &temp = reinterpret_cast<cv::Mat_<cv::Vec3b>&>(img); // Appropriately type pixels - allows for
temp(j,i)[k] for k =0,1,2
