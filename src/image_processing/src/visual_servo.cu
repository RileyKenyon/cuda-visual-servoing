...
std::optional<cv::Point> VisualServo::process_frame(const cv::Mat &img) {
  if (img.empty() || !img.isContinuous() || img.rows != static_cast<int>(imageHeight) ||
      img.cols != static_cast<int>(imageWidth) || img.channels() != static_cast<int>(imageLayers)) {
    return std::nullopt;
  }
  if (!process(img.data)) {
    return std::nullopt;
  }
  // Stub for now - only return the inner 80% of the screen
  static int count = 0;
  cv::Point p{count % imageWidth, (count % (imageHeight / 2)) + 0.2 * imageHeight};
  ++count;
  return p;
}
...
void VisualServo::get_framerate() {
  ...
  if (reportFramerate && (0 == counter % frameWindow)) {
    ...
    std::cout << "FPS GPU: " << (frameWindow / timer) * 1000.0 << std::endl;
    timer = 0.0f;
    counter = 0;
  } else {
    counter++;
