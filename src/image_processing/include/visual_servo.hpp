...
public:
  ...
  /// Process one frame and optionally get tap location.
  /// @param img Input image (must match constructor width/height/channels and be continuous).
  /// @param x_norm Output: tap x in normalized image coords 0-100 (only set when returning true).
  /// @param y_norm Output: tap y in normalized image coords 0-100 (only set when returning true).
  /// @return true if a tap location is available, false if no tap (or invalid input).
  std::optional<cv::Point> process_frame(const cv::Mat &img);
