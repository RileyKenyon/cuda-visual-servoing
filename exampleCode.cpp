...
{
    const char* gst;
    //gst = "nvcamerasrc ! 'video/x-raw(memory:NVMM), width=(int)1920, height=(int)1080, format=(string)I420, framerate=(fraction)60/1' ! nvvidconv ! 'video/x-raw(memory:NVMM), format=(string)I420' ! nvoverlaysink -e";
    //gst = "nvcamerasrc ! 'video/x-raw, format=(string)BGR, framerate=(fraction)[ 0/1, 2147483647/1 ], width=(int)2592, height=(int)1944, interlace-mode=(string)progressive, pixel-aspect-ratio=(fraction)1/1, colorimetry=(string)sRGB' ! nvoverlaysink -e";
    //gst = "imxv4l2videosrc device = \"/dev/video0\" ! videoconvert ! appsink";
